...
class Delft_IVVI_ChannelModule(InstrumentChannel):
    def __init__(self, parent:VisaInstrument, channel_num, name, numdacs=16):
        ...
        self.add_parameter('voltage_ramp_rate', unit='mV/s',
                label="Output voltage ramp-rate",
                initial_value=2.5e-3/0.05, #?
                vals=vals.Numbers(0.001, 1),
                get_cmd=lambda : self.voltage.step/self.voltage.inter_delay*1e-3,
                set_cmd=self._set_ramp_rate)
            

    def _set_voltage(self, voltage):
        result = self._parent._set_dac(self._channel_num, voltage)        
        return result
    
    def _get_voltage(self):
        result = self._parent._get_dac(self._channel_num)
        return result
    
    def _set_voltage_bounds(self):
        polarity = self._parent.get_pol_dac(self._channel_num)
        val =  self._parent.set_pol_dacrack(polarity, self._channel_num, get_all = False)
        self.voltage.vals = vals.Numbers(val, val+self.full_range)

    def _set_ramp_rate(self, ramp_rate):
        if ramp_rate < 0.01:
            self.voltage.step = 1 #1mV
        elif ramp_rate < 0.1:
            self.voltage.step = 10
        elif ramp_rate < 1.0:
            self.voltage.step = 100
        else:
            self.voltage.step = 1000 #1V
        self.voltage.inter_delay = self.voltage.step / ramp_rate*1e-3
...
class VOLT_Delft_IVVI(VisaInstrument):
    ...
    def __init__(self, name, address, numdacs=16, safe_version=True,
        ...
        #adding DAC channels as submodules
        channels = ChannelList(self, "DAC", Delft_IVVI_ChannelModule, snapshotable=False)
        for ch_idx in range(1, self._numdacs+1):
            ch_name = f'DAC{ch_idx}'
            cur_module = Delft_IVVI_ChannelModule(self, ch_idx, ch_name)
            self.add_submodule(ch_name, cur_module)
            channels.append(cur_module)
...
if __name__ == "__main__":
    temp1 = VOLT_Delft_IVVI('parent', 'ASRL1')
    a=0
...
#     def __init__(self, name, address, channel=0 ,reset=False, numdacs=4, dac_step=10,
...
# if __name__ == "__main__":
#     temp = VOLT_Delft_IVVI('terst', 'ASRL1',)
#     temp.dac2(0)
#     temp.dac2()
#     a=0
