...
class GPU_ConstantArithmetic(ProcNodeGPU):
    ...
    def process_data(self, data_pkt, **kwargs):
        ...
        for ch_ind, cur_ch in enumerate(data_pkt['data'].keys()):
            if self.channels == None:
                data_pkt['data'][cur_ch] = self.perform_arithmetic(ProcNodeGPU.check_conv_to_cupy_array(data_pkt['data'][cur_ch]), self.operation, self.constant)
